...
import { 
  Search, ChevronLeft, Plus, LogOut, X, Check, SlidersHorizontal, Scissors, Home, Building, Info, GraduationCap, RefreshCcw, Sun, Moon, AlertTriangle, Loader2, Clock, Map as MapIcon, List, Globe, Calendar
} from 'lucide-react';
...
const EXTERNSHIP_YEARS = Array.from({ length: 5 }, (_, i) => new Date().getFullYear() - i);
...
const RUBRIC: Record<string, { label: string, low: string, high: string }> = {
  ...
  culture: { label: "Culture", low: "1: Toxic environment.", high: "5: Student-focused team." },
  volume: { label: "Caseload", low: "1: Slow/repetitive.", high: "5: High volume/variety." }
};
...
export default function MapContainer() {
  ...
  const [newClinic, setNewClinic] = useState({
    ...
    ownership_type: 'Private Practice', 
    website: '', 
  ...
  const [ratings, setRatings] = useState({ 
    ...
    open_to_years: [] as string[],
    duration_weeks: 2,
    externship_year: new Date().getFullYear() 
  });
  ...
  const addLayers = useCallback((m: maplibregl.Map, data: any[]) => {
    ...
    m.addSource('clinics', { 
      type: 'geojson', 
      data: { type: 'FeatureCollection', features: data.map(c => ({ type: 'Feature', geometry: { type: 'Point', coordinates: [c.longitude, c.latitude] }, properties: { fullData: JSON.stringify(c) } })) } 
    });
    m.addLayer({ 
      id: 'p', type: 'circle', source: 'clinics', 
      paint: { 'circle-color': '#64d2b1', 'circle-radius': 10, 'circle-stroke-width': 2, 'circle-stroke-color': isDarkMode ? '#ffffff' : '#1a1a1a' } 
    });
  }, [isDarkMode]);
  ...
  useEffect(() => {
    ...
    if (s) {
      s.setData({ type: 'FeatureCollection', features: filteredClinics.map(c => ({ type: 'Feature', geometry: { type: 'Point', coordinates: [c.longitude, c.latitude] }, properties: { fullData: JSON.stringify(c) } })) });
    }
  ...
  const stateSuggestions = useMemo(() => {
    const base = clinics.filter(c => countryFilter === 'All' || c.country === countryFilter);
    return [...new Set(base.map(c => c.state).filter(Boolean))].filter(s => s.toLowerCase().includes(stateInput.toLowerCase()));
  }, [clinics, countryFilter, stateInput]);
  const citySuggestions = useMemo(() => {
    const base = clinics.filter(c => (countryFilter === 'All' || c.country === countryFilter) && (stateFilter === 'All' || c.state === stateFilter));
    return [...new Set(base.map(c => c.city).filter(Boolean))].filter(ct => ct.toLowerCase().includes(cityInput.toLowerCase()));
  }, [clinics, countryFilter, stateFilter, cityInput]);
  ...
  const submitClinic = async () => {
    ...
    try {
      const { error } = await supabase.from('clinics').insert({ 
        ...
        latitude: newClinic.latitude, longitude: newClinic.longitude, 
        category: newClinic.ownership_type, 
      ...
      if (error) { console.error(error); alert(`Error: ${error.message}`); } 
        ...
        setNewClinic({ name: '', address: '', city: '', state: '', postal_code: '', country: '', ownership_type: 'Private Practice', website: '', specialties: [], animal_types: [], latitude: 0, longitude: 0 });
        setAddressQuery('');
        alert("Clinic submitted for verification!");
      }
  ...
  return (
    <div className={`relative w-full h-[100dvh] font-sans antialiased overflow-hidden transition-colors duration-500 ${isDarkMode ? 'bg-charcoal text-white' : 'bg-slate-50 text-slate-900'}`}>
      ...
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-30 md:hidden flex gap-2">
         {showMobileMap && (
             <button onClick={() => setShowMobileMap(false)} className={`flex items-center gap-2 px-6 py-3 rounded-full font-bold shadow-2xl animate-in fade-in slide-in-from-bottom-10 cursor-pointer active:scale-95 transition-transform ${isDarkMode ? 'bg-vet-mint text-charcoal' : 'bg-emerald-600 text-white'}`}>
      ...
      {showLightWarning && (
        <div className="absolute top-6 left-1/2 -translate-x-1/2 z-[100] bg-yellow-500 text-black px-6 py-3 rounded-full font-black uppercase text-[10px] flex items-center gap-3 shadow-2xl animate-in fade-in slide-in-from-top-4 duration-300">
          <AlertTriangle size={16} /> Light Mode? really?
        </div>
      )}
        ...
        <div className={`h-full w-full relative overflow-hidden md:rounded-[2rem] border-l md:border shadow-2xl backdrop-blur-xl flex flex-col transition-all ${isDarkMode ? 'bg-black/85 border-white/20' : 'bg-white/95 border-black/10'}`}>
          ...
          <div className={`p-4 md:p-6 border-b flex items-center justify-between gap-2 ${isDarkMode ? 'border-white/10' : 'border-black/5'}`}>
            ...
            <div className="flex items-center gap-2 shrink-0">
              <button onClick={() => setShowMobileMap(true)} className={`md:hidden p-2 rounded-full transition-transform active:scale-90 cursor-pointer ${isDarkMode ? 'bg-white/5 text-white' : 'bg-black/5 text-black'}`}>
                 <MapIcon size={18} />
              </button>
              <button onClick={toggleTheme} className={`p-2 rounded-full transition-transform hover:scale-110 cursor-pointer ${isDarkMode ? 'bg-white/5 text-white' : 'bg-black/5 text-black'}`}>
                {isDarkMode ? <Sun size={18} /> : <Moon size={18} />}
              </button>
              <button onClick={() => setIsAddingClinic(true)} className={`p-2 rounded-full hover:scale-110 transition-transform cursor-pointer ${isDarkMode ? 'bg-vet-mint text-charcoal' : 'bg-emerald-600 text-white'}`}>
                <Plus size={18} />
              </button>
              {userId ? (
                <button onClick={() => supabase.auth.signOut().then(() => window.location.reload())} className={`p-2 rounded-full hover:bg-opacity-20 transition-colors cursor-pointer ${isDarkMode ? 'bg-white/5' : 'bg-black/5'}`}><LogOut size={18}/></button>
              ) : (
                <button onClick={() => setIsAuthOpen(true)} className={`px-3 md:px-4 py-2 border rounded-full text-[10px] font-black uppercase transition-all cursor-pointer hover:scale-105 active:scale-95 ${isDarkMode ? 'bg-vet-mint/10 border-vet-mint text-vet-mint hover:bg-vet-mint/20' : 'bg-emerald-600/10 border-emerald-600 text-emerald-600 hover:bg-emerald-600/20'}`}>Login</button>
              )}
            </div>
          </div>
          ...
          <div className="relative flex-1 overflow-hidden">
            {isAddingClinic ? (
              <div className={`absolute inset-0 z-20 p-6 overflow-y-auto animate-in slide-in-from-bottom duration-300 scrollbar-hide ${isDarkMode ? 'bg-charcoal/95' : 'bg-slate-50/95'}`}>
                <div className="flex justify-between items-center mb-6"><h3 className="text-lg font-black italic uppercase">Add Location</h3><button onClick={() => setIsAddingClinic(false)} className="cursor-pointer hover:scale-110 active:scale-90 transition-transform"><X size={20}/></button></div>
                <div className="space-y-4 pb-4 border-b border-dashed border-opacity-20 mb-4 border-gray-500">
                    <div className="relative">
                        <label className="text-[10px] font-bold opacity-30 uppercase block mb-1">Step 1: Search Address</label>
                        <div className="relative">
                            <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 opacity-30"/>
                            <input placeholder="Search by name or address..." className={`pl-9 ${inputStyle}`} value={addressQuery} onChange={e => { setAddressQuery(e.target.value); setShowAddressSuggestions(true); }} />
                        </div>
                        {showAddressSuggestions && addressSuggestions.length > 0 && (
                            <div className={`absolute top-full left-0 w-full mt-2 border rounded-xl z-50 overflow-hidden shadow-2xl ${isDarkMode ? 'bg-black border-white/10' : 'bg-white border-black/10'}`}>
                                {addressSuggestions.map((s, i) => (<button key={i} type="button" onClick={() => selectAddress(s)} className={`w-full text-left p-3 border-b text-xs last:border-0 transition-colors cursor-pointer ${isDarkMode ? 'hover:bg-vet-mint/20 border-white/5' : 'hover:bg-emerald-600/10 border-black/5'}`}><span className="font-bold block text-[11px] mb-0.5">{s.name || s.display_name.split(',')[0]}</span><span className="opacity-50 text-[10px] block truncate">{s.display_name}</span></button>))}
                            </div>
                        )}
                    </div>
                </div>
                {newClinic.latitude !== 0 && (
                    <div className="space-y-4 pb-20 animate-in fade-in slide-in-from-bottom-4">
                        <div className={`p-4 rounded-xl border ${isDarkMode ? 'bg-vet-mint/5 border-vet-mint/20' : 'bg-emerald-600/5 border-emerald-600/20'}`}>
                            <p className="text-[10px] font-bold uppercase opacity-50 mb-2 flex items-center gap-1"><Check size={12}/> Coordinates Locked</p>
                            <p className="text-[10px] font-mono opacity-70">{newClinic.latitude.toFixed(6)}, {newClinic.longitude.toFixed(6)}</p>
                        </div>
                        ...
                        <div><label className="text-[10px] font-bold opacity-30 uppercase block mb-1">Website / Externship Page (Optional)</label><input placeholder="https://..." value={newClinic.website} onChange={e => setNewClinic({...newClinic, website: e.target.value})} className={inputStyle} /></div>
                        ...
                        <div><label className="text-[10px] font-bold opacity-30 uppercase block mb-2">Specialty (Select all that apply)</label><div className="flex flex-wrap gap-2">{SPECIALTIES.map(s => {const isActive = newClinic.specialties.includes(s);return (<button key={s} onClick={() => setNewClinic(prev => ({...prev, specialties: isActive ? prev.specialties.filter(x => x !== s) : [...prev.specialties, s]}))} className={`px-2 py-1 rounded-md text-[10px] border cursor-pointer hover:opacity-80 active:scale-95 transition-all ${isActive ? (isDarkMode ? 'bg-vet-mint text-charcoal' : 'bg-emerald-600 text-white') : 'border-white/10 opacity-50'}`}>{s}</button>)})}</div></div>
                        ...
                        <button onClick={submitClinic} disabled={isSubmitting} className={`w-full py-4 rounded-2xl font-black uppercase text-[11px] shadow-lg mt-4 flex justify-center items-center gap-2 cursor-pointer hover:shadow-xl hover:-translate-y-0.5 active:translate-y-0 active:scale-[0.99] transition-all ${isDarkMode ? 'bg-vet-mint text-charcoal' : 'bg-emerald-600 text-white'} ${isSubmitting ? 'opacity-50 cursor-not-allowed' : ''}`}>{isSubmitting ? <Loader2 className="animate-spin" size={14} /> : 'Confirm & Submit Location'}</button>
                    </div>
                )}
              </div>
            ) : (
                ...
                <div className={`p-6 border-b space-y-4 ${isDarkMode ? 'border-white/5' : 'border-black/5'}`}>
                  <div className="flex gap-2">
                    <div className="relative flex-1">
                      ...
                      <input placeholder="Search hospital name..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className={`w-full pl-10 pr-4 py-3 border rounded-xl text-[16px] md:text-[13px] outline-none transition-all ${isDarkMode ? 'bg-white/5 border-white/10 focus:border-vet-mint/30' : 'bg-black/5 border-black/10 focus:border-emerald-600/30'}`} />
                    </div>
                    <button onClick={() => setShowFilters(!showFilters)} className={`p-3 rounded-xl border transition-all cursor-pointer hover:scale-105 active:scale-95 ${showFilters ? (isDarkMode ? 'bg-vet-mint border-vet-mint text-charcoal' : 'bg-emerald-600 border-emerald-600 text-white') : (isDarkMode ? 'bg-white/5 border-white/10 text-white/40' : 'bg-black/5 border-black/10 text-black/40')}`}><SlidersHorizontal size={18} /></button>
                  </div>
                  {showFilters && (
                    <div className="space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
                      <div className="grid grid-cols-1 gap-2">
                        <div className="relative"><input placeholder="Country..." className={`w-full p-3 rounded-xl text-[11px] outline-none border transition-all ${isDarkMode ? 'bg-white/5 border-white/10 focus:border-vet-mint/30' : 'bg-black/5 border-black/10 focus:border-emerald-600/30'}`} value={countryInput} onChange={e => { setCountryInput(e.target.value); setShowCountrySuggestions(true); }} onBlur={() => setTimeout(() => setShowCountrySuggestions(false), 200)} />{showCountrySuggestions && countrySuggestions.length > 0 && <div className={`absolute z-50 top-full left-0 w-full mt-1 border rounded-xl overflow-hidden shadow-xl ${isDarkMode ? 'bg-black border-white/10' : 'bg-white border-black/10'}`}>{countrySuggestions.map(c => (<button key={c} className={`w-full text-left p-3 text-[10px] border-b last:border-0 hover:bg-white/10 cursor-pointer ${isDarkMode ? 'text-white border-white/5' : 'text-black border-black/5'}`} onClick={() => { setCountryFilter(c); setCountryInput(c); }}>{c}</button>))}</div>}</div>
                        <div className="relative"><input placeholder="State..." className={`w-full p-3 rounded-xl text-[11px] outline-none border transition-all ${isDarkMode ? 'bg-white/5 border-white/10 focus:border-vet-mint/30' : 'bg-black/5 border-black/10 focus:border-emerald-600/30'}`} value={stateInput} onChange={e => { setStateInput(e.target.value); setShowStateSuggestions(true); }} onBlur={() => setTimeout(() => setShowStateSuggestions(false), 200)} />{showStateSuggestions && stateSuggestions.length > 0 && <div className={`absolute z-50 top-full left-0 w-full mt-1 border rounded-xl overflow-hidden shadow-xl ${isDarkMode ? 'bg-black border-white/10' : 'bg-white border-black/10'}`}>{stateSuggestions.map(s => (<button key={s} className={`w-full text-left p-3 text-[10px] border-b last:border-0 hover:bg-white/10 cursor-pointer ${isDarkMode ? 'text-white border-white/5' : 'text-black border-black/5'}`} onClick={() => { setStateFilter(s); setStateInput(s); }}>{s}</button>))}</div>}</div>
                        <div className="relative"><input placeholder="City..." className={`w-full p-3 rounded-xl text-[11px] outline-none border transition-all ${isDarkMode ? 'bg-white/5 border-white/10 focus:border-vet-mint/30' : 'bg-black/5 border-black/10 focus:border-emerald-600/30'}`} value={cityInput} onChange={e => { setCityInput(e.target.value); setShowCitySuggestions(true); }} onBlur={() => setTimeout(() => setShowCitySuggestions(false), 200)} />{showCitySuggestions && citySuggestions.length > 0 && <div className={`absolute z-50 top-full left-0 w-full mt-1 border rounded-xl overflow-hidden shadow-xl ${isDarkMode ? 'bg-black border-white/10' : 'bg-white border-black/10'}`}>{citySuggestions.map(c => (<button key={c} className={`w-full text-left p-3 text-[10px] border-b last:border-0 hover:bg-white/10 cursor-pointer ${isDarkMode ? 'text-white border-white/5' : 'text-black border-black/5'}`} onClick={() => { setCityFilter(c); setCityInput(c); }}>{c}</button>))}</div>}</div>
                      </div>
                      <div>
                        <p className="text-[10px] font-bold uppercase opacity-40 mb-2">Animal Focus</p>
                        <div className="flex flex-wrap gap-2">
                           <button onClick={() => setAnimalFocusFilter('All')} className={`px-3 py-1 rounded-full text-[10px] font-bold border transition-colors cursor-pointer hover:bg-white/10 active:scale-95 ${animalFocusFilter === 'All' ? (isDarkMode ? 'bg-vet-mint text-charcoal border-vet-mint' : 'bg-emerald-600 text-white border-emerald-600') : 'border-white/10 opacity-50'}`}>All</button>
                  ...
                  {filteredClinics.map(clinic => (
                    <button key={clinic.id} onClick={() => { if (map.current) { map.current.flyTo({ center: [clinic.longitude, clinic.latitude], zoom: 14, speed: 1.5, essential: true }); } setSelectedClinic(clinic); }} className={`w-full text-left p-5 rounded-3xl border border-transparent transition-all flex justify-between items-start group cursor-pointer hover:scale-[1.01] active:scale-[0.99] shadow-sm hover:shadow-md ${isDarkMode ? 'hover:bg-white/5 hover:border-white/10' : 'hover:bg-black/5 hover:border-black/10'}`}>
                        <div className="max-w-[70%]">
                            <h3 className={`font-bold text-[14px] truncate transition-colors ${isDarkMode ? 'group-hover:text-vet-mint' : 'group-hover:text-emerald-600'}`}>{clinic.name}</h3>
                            <p className="text-[10px] font-bold opacity-30 uppercase">{clinic.city}, {clinic.country}</p>
                            <div className="flex flex-wrap gap-1 mt-2">{clinic.animal_types?.slice(0, 3).map((t: string) => (<span key={t} className={`text-[9px] px-1.5 py-0.5 rounded-md font-bold tracking-tight opacity-70 ${isDarkMode ? 'bg-white/10 text-white' : 'bg-black/5 text-black'}`}>{t}</span>))}</div>
                        </div>
            ...
            {selectedClinic && (
              <div className={`absolute inset-0 flex flex-col animate-in slide-in-from-right duration-500 overflow-hidden scrollbar-hide ${isDarkMode ? 'bg-charcoal/40' : 'bg-slate-100/40'}`}>
                <div className={`p-4 flex items-center gap-3 border-b ${isDarkMode ? 'border-white/10 bg-black/20' : 'border-black/10 bg-white/20'}`}>
                    <button onClick={() => { setSelectedClinic(null); setIsReviewing(false); }} className={`p-2 rounded-full transition-colors cursor-pointer hover:scale-110 active:scale-90 ${isDarkMode ? 'hover:bg-white/10' : 'hover:bg-black/10'}`}><ChevronLeft size={20} /></button>
                  ...
                  {!isReviewing ? (
                    <div className="space-y-6 pb-20">
                      ...
                      {selectedClinic.website && (
                        <a href={selectedClinic.website} target="_blank" rel="noopener noreferrer" className={`w-full py-3 rounded-2xl flex items-center justify-center gap-2 font-bold text-xs uppercase tracking-widest border transition-all cursor-pointer hover:scale-[1.02] active:scale-[0.98] ${isDarkMode ? 'bg-white/10 border-white/10 hover:bg-white/20' : 'bg-black/5 border-black/5 hover:bg-black/10'}`}>
                            <Globe size={14}/> Visit Website
                        </a>
                      )}

                      <div className="grid grid-cols-2 gap-4">{Object.entries(RUBRIC).map(([key, info], idx) => (<div key={key} className={`p-4 rounded-3xl border relative group ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-black/5 border-black/10'}`}><div className="flex justify-between items-start mb-1"><p className="text-[10px] uppercase font-bold opacity-30 tracking-widest">{info.label}</p><Info size={12} className="opacity-20" /></div><p className={`text-2xl font-black ${isDarkMode ? 'text-vet-mint' : 'text-emerald-600'}`}>{getAvg(selectedClinic.reviews, key)}</p></div>))}</div>
                      ...
                      <div className="space-y-2">
                         <div className={`p-4 rounded-2xl border flex items-center justify-between transition-all ${selectedClinic.provides_stipend ? (isDarkMode ? 'border-vet-mint text-vet-mint bg-vet-mint/5' : 'border-emerald-600 text-emerald-600 bg-emerald-600/5') : 'opacity-20 border-gray-500'}`}><span className="text-xs font-bold uppercase">Stipend Provided</span>{selectedClinic.provides_stipend ? <Check size={16}/> : <Home size={16} />}</div>
                         <div className={`p-4 rounded-2xl border flex items-center justify-between transition-all ${selectedClinic.allows_surgery ? (isDarkMode ? 'border-vet-mint text-vet-mint bg-vet-mint/5' : 'border-emerald-600 text-emerald-600 bg-emerald-600/5') : 'opacity-20 border-gray-500'}`}><span className="text-xs font-bold uppercase">Hands-on Surgery</span>{selectedClinic.allows_surgery ? <Check size={16}/> : <Scissors size={16} />}</div>
                         <div className={`p-4 rounded-2xl border flex items-center justify-between transition-all ${selectedClinic.reviews?.some((r: any) => r.hosts_intern_residents) ? (isDarkMode ? 'border-vet-mint text-vet-mint bg-vet-mint/5' : 'border-emerald-600 text-emerald-600 bg-emerald-600/5') : 'opacity-20 border-gray-500'}`}><span className="text-xs font-bold uppercase">Hosts Interns/Residents</span>{selectedClinic.reviews?.some((r: any) => r.hosts_intern_residents) ? <Check size={16}/> : <Building size={16} />}</div>
                      </div>
                      <div className="mt-4 pt-4 border-t border-dashed border-opacity-20 border-gray-500">
                          <h4 className="text-[10px] font-bold uppercase opacity-50 mb-4">Recent Reviews</h4>
                          {selectedClinic.reviews && selectedClinic.reviews.length > 0 ? (
                            selectedClinic.reviews.map((r: any, i: number) => (
                                <div key={i} className={`p-5 rounded-2xl mb-3 border ${isDarkMode ? 'bg-white/5 border-white/5' : 'bg-black/5 border-black/5'}`}>
                                    <div className="flex justify-between items-center mb-2">
                                        <div className="flex items-center gap-2">
                                            <span className={`text-lg font-black ${isDarkMode ? 'text-vet-mint' : 'text-emerald-600'}`}>{r.overall_rating}★</span>
                                            {r.duration_weeks && <span className="text-[9px] px-2 py-1 rounded-full bg-white/10 flex items-center gap-1"><Clock size={8}/> {r.duration_weeks} wks</span>}
                                            {r.externship_year && <span className="text-[9px] px-2 py-1 rounded-full bg-white/10 flex items-center gap-1"><Calendar size={8}/> {r.externship_year}</span>}
                                        </div>
                                        <span className="text-[9px] opacity-40">{new Date(r.created_at).toLocaleDateString()}</span>
                                    </div>
                                    <p className="text-xs opacity-80 leading-relaxed italic">"{r.comment}"</p>
                                </div>
                            ))
                          ) : (
                             <div className="text-center p-8 opacity-40 text-xs italic">No reviews yet. Be the first!</div>
                          )}
                      </div>
                      <button onClick={() => !userId ? setIsAuthOpen(true) : setIsReviewing(true)} className={`w-full py-4 rounded-2xl font-black uppercase text-[11px] shadow-lg transition-transform hover:scale-[1.02] active:scale-[0.98] cursor-pointer ${isDarkMode ? 'bg-vet-mint text-charcoal shadow-vet-mint/20' : 'bg-emerald-600 text-white shadow-emerald-600/20'}`}>Add Review</button>
                    </div>
                  ) : (
                    <div className="space-y-6 pb-20 animate-in slide-in-from-right duration-300">
                       <div className={`p-5 border rounded-3xl text-center ${isDarkMode ? 'bg-vet-mint/5 border-vet-mint/20' : 'bg-emerald-600/5 border-emerald-600/20'}`}>
                            <label className={`text-[11px] font-black uppercase block mb-2 tracking-widest ${isDarkMode ? 'text-vet-mint' : 'text-emerald-600'}`}>Overall experience</label>
                            <input type="range" min="1" max="5" value={ratings.overall_rating} onChange={e => setRatings({...ratings, overall_rating: parseInt(e.target.value)})} className={`w-full cursor-pointer accent-current ${isDarkMode ? 'text-vet-mint' : 'text-emerald-600'}`} />
                            <div className={`text-3xl font-black mt-2 ${isDarkMode ? 'text-vet-mint' : 'text-emerald-600'}`}>{ratings.overall_rating} ★</div>
                       </div>
                       
                       <div className={`p-5 border rounded-3xl ${isDarkMode ? 'bg-white/5 border-white/5' : 'bg-black/5 border-black/5'}`}>
                            <div className="flex justify-between items-center mb-2">
                                <label className="text-[11px] font-bold uppercase opacity-60">Duration of Externship</label>
                                <span className={`text-[12px] font-black ${isDarkMode ? 'text-vet-mint' : 'text-emerald-600'}`}>{ratings.duration_weeks} Weeks</span>
                            </div>
                            <input type="range" min="1" max="4" step="1" value={ratings.duration_weeks} onChange={e => setRatings({...ratings, duration_weeks: parseInt(e.target.value)})} className={`w-full h-1.5 appearance-none rounded-lg cursor-pointer ${isDarkMode ? 'bg-white/10 accent-white' : 'bg-black/10 accent-black'}`} />
                            <div className="flex justify-between text-[9px] opacity-30 mt-2"><span>1 Week</span><span>4 Weeks+</span></div>
                       </div>

                       <div>
                          <label className="text-[10px] font-bold opacity-30 uppercase block mb-2">Year of Externship</label>
                          <select value={ratings.externship_year} onChange={(e) => setRatings({...ratings, externship_year: parseInt(e.target.value)})} className={`w-full p-3 rounded-xl text-[12px] outline-none border cursor-pointer ${isDarkMode ? 'bg-white/10 border-white/10 text-white' : 'bg-black/5 border-black/10 text-black'}`}>
                             {EXTERNSHIP_YEARS.map(y => <option key={y} value={y} className="text-black">{y}</option>)}
                          </select>
                       </div>

                       {Object.entries(RUBRIC).map(([key, info]) => (
                         <div key={key} className={`p-5 rounded-3xl border space-y-4 relative ${isDarkMode ? 'bg-white/5 border-white/5' : 'bg-black/5 border-black/5'}`}>
                           <div className="flex justify-between items-center"><div className="flex items-center gap-2"><label className="text-[11px] font-bold uppercase opacity-60">{info.label}</label><span className={`text-[11px] font-black ${isDarkMode ? 'text-vet-mint' : 'text-emerald-600'}`}>{(ratings as any)[key]} / 5</span></div></div>
                           <input type="range" min="1" max="5" value={(ratings as any)[key]} onChange={e => setRatings({...ratings, [key]: parseInt(e.target.value)})} className={`w-full h-1.5 appearance-none rounded-lg cursor-pointer ${isDarkMode ? 'bg-white/10 accent-white' : 'bg-black/10 accent-black'}`} />
                         </div>
                       ))}
                       ...
                       <div className="grid gap-3">
                          <button type="button" onClick={() => setRatings({...ratings, allows_surgery: !ratings.allows_surgery})} className={`p-4 rounded-2xl border flex items-center justify-between transition-all cursor-pointer hover:opacity-80 active:scale-95 ${ratings.allows_surgery ? (isDarkMode ? 'bg-vet-mint/10 border-vet-mint text-vet-mint' : 'bg-emerald-600/10 border-emerald-600 text-emerald-600') : (isDarkMode ? 'bg-white/5 border-white/10 opacity-40' : 'bg-black/5 border-black/10 opacity-40')}`}>Surgery allowed? {ratings.allows_surgery ? <Check size={16}/> : <Scissors size={16}/>}</button>
                          <button type="button" onClick={() => setRatings({...ratings, hosts_intern_residents: !ratings.hosts_intern_residents})} className={`p-4 rounded-2xl border flex items-center justify-between transition-all cursor-pointer hover:opacity-80 active:scale-95 ${ratings.hosts_intern_residents ? (isDarkMode ? 'bg-vet-mint/10 border-vet-mint text-vet-mint' : 'bg-emerald-600/10 border-emerald-600 text-emerald-600') : (isDarkMode ? 'bg-white/5 border-white/10 opacity-40' : 'bg-black/5 border-black/10 opacity-40')}`}>Hosts Interns? {ratings.hosts_intern_residents ? <Check size={16}/> : <Building size={16}/>}</button>
                          <button type="button" onClick={() => setRatings({...ratings, provides_stipend: !ratings.provides_stipend})} className={`p-4 rounded-2xl border flex items-center justify-between transition-all cursor-pointer hover:opacity-80 active:scale-95 ${ratings.provides_stipend ? (isDarkMode ? 'bg-vet-mint/10 border-vet-mint text-vet-mint' : 'bg-emerald-600/10 border-emerald-600 text-emerald-600') : (isDarkMode ? 'bg-white/5 border-white/10 opacity-40' : 'bg-black/5 border-black/10 opacity-40')}`}>Stipend provided? {ratings.provides_stipend ? <Check size={16}/> : <Home size={16}/>}</button>
                       </div>
                       ...
                       <button onClick={submitReview} className={`w-full py-4 rounded-2xl font-black uppercase text-[11px] cursor-pointer hover:shadow-lg hover:-translate-y-0.5 active:translate-y-0 active:scale-[0.99] transition-all ${isDarkMode ? 'bg-vet-mint text-charcoal' : 'bg-emerald-600 text-white'}`}>Submit Review</button>
                    </div>
                  )}
                </div>
              </div>
