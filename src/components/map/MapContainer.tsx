...
// ... (Rest of your imports and constants: supabase, VET_YEARS, etc. - NO CHANGES HERE)
...
export default function MapContainer() {
  ...
  const [isAboutOpen, setIsAboutOpen] = useState(false); // NEW: About Modal State
  ...
  // ... (Rest of your state: isManualMode, tooltipState, addressQuery, etc.)
  ...
  useEffect(() => {
    ...
    m.on('mouseenter', 'p', (e: any) => {
        ...
        const avgRating = reviews.length > 0 ? (reviews.reduce((acc: number, r: any) => acc + (r.overall_rating || 0), 0) / reviews.length).toFixed(1) : null;
        if (popupRef.current) popupRef.current.remove();
        popupRef.current = new maplibregl.Popup({ closeButton: false, closeOnClick: false, offset: 15, className: 'custom-map-popup' })
            ...
            .setHTML(`
                <div style="font-family: sans-serif; padding: 2px;">
                    ...
                    <div style="display: flex; align-items: center; gap: 6px;">
                        ${avgRating ? `<div style="background-color: #d1fae5; color: #047857; font-size: 10px; font-weight: 800; padding: 2px 6px; border-radius: 4px;">★ ${avgRating}</div>` : ''}
    ...
    m.on('mouseleave', 'p', () => { m.getCanvas().style.cursor = ''; if (popupRef.current) popupRef.current.remove(); });
  ...
  const submitClinic = async () => {
    ...
    if (!finalLat || !finalLng) {
        ...
        alert("We couldn't locate this address automatically. Please ensure the Address, City, and Country are correct.");
        return;
    }
  ...
  const LoginWarning = () => (
    !userId ? (
        <div className={`p-6 mt-4 text-center rounded-2xl border border-dashed ${isDarkMode ? 'border-white/10 bg-white/5' : 'border-black/10 bg-black/5'}`}>
            ...
            <p className="text-[10px] mt-2 opacity-60 max-w-[200px] mx-auto">Sign in to add reviews, submit clinics, and vote.</p>
        </div>
    ) : null
  );
    ...
    <div className={`relative w-full h-dvh min-h-screen font-sans antialiased overflow-hidden transition-colors duration-500 ${isDarkMode ? 'bg-charcoal text-white' : 'bg-slate-50 text-slate-900'}`} onClick={hideTooltip}>
      ...
      {isAboutOpen && (
        <div className="fixed inset-0 z-200 flex items-center justify-center p-6 bg-black/80 backdrop-blur-md animate-in fade-in duration-300">
            <div className={`relative w-full max-w-lg p-8 rounded-4xl border shadow-2xl ${isDarkMode ? 'bg-[#0f0f0f] border-white/10 text-white' : 'bg-white border-black/10 text-slate-900'}`}>
                <button onClick={() => setIsAboutOpen(false)} className={`absolute top-4 right-4 p-2 rounded-full hover:bg-white/10 ${isDarkMode ? 'text-white' : 'text-black'}`}><X size={20} /></button>
                
                <h2 className={`text-xl font-black uppercase italic mb-4 ${isDarkMode ? 'text-vet-mint' : 'text-emerald-600'}`}>Why I Built This</h2>
                <p className="text-sm leading-relaxed mb-6 opacity-80">
                    As a veterinary student, I realized how difficult it was to find annecdotal information about externship locations. We rely so much on word-of-mouth, that advise from previous students often gets lost to time.
                    <br/><br/>
                ...
                <h3 className="text-sm font-bold uppercase mb-2 opacity-50">How to use</h3>
                <ul className="space-y-3 text-sm opacity-80 mb-8">
                    <li className="flex gap-3"><MapIcon size={16} className="shrink-0 text-vet-mint"/> <span><strong>Browse the Map:</strong> Explore clinics visually or switch to the list view to filter by stipend, housing, and more.</span></li>
                    <li className="flex gap-3"><Plus size={16} className="shrink-0 text-vet-mint"/> <span><strong>Add Locations:</strong> If your externship isn't listed, add it! It helps the community grow.</span></li>
                    <li className="flex gap-3"><Stethoscope size={16} className="shrink-0 text-vet-mint"/> <span><strong>Leave Reviews:</strong> Anonymous, honest feedback helps future students know what to expect. Just be professional about it.</span></li>
                </ul>

                <button onClick={() => setIsAboutOpen(false)} className={`w-full py-4 rounded-xl font-black uppercase tracking-widest text-xs transition-all hover:scale-[1.02] active:scale-[0.98] ${isDarkMode ? 'bg-vet-mint text-charcoal shadow-vet-mint/20 shadow-lg' : 'bg-emerald-600 text-white shadow-emerald-600/20 shadow-lg'}`}>Got it!</button>
            </div>
        </div>
      )}
          ...
          <div className="relative flex-1 overflow-hidden">
            ...
            <div className={`absolute inset-0 flex flex-col bg-inherit transition-all duration-500 ease-in-out ${selectedClinic || isAddingClinic ? 'opacity-0 pointer-events-none scale-95' : 'opacity-100 scale-100 pointer-events-auto'}`} style={{touchAction: 'pan-y'}}>
                <div className={`p-6 border-b space-y-6 ${isDarkMode ? 'border-white/5' : 'border-black/5'}`}>
                  {/* ... (Search and Filter Bar same as before, ensuring inputs are text-base) ... */}
                  <div className="flex gap-2"><div className="relative flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 opacity-30" size={16} /><input placeholder="Search hospital name..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className={`w-full pl-10 pr-4 py-3 border rounded-xl text-base md:text-[13px] outline-none transition-all ${isDarkMode ? 'bg-white/5 border-white/10 focus:border-vet-mint/30' : 'bg-black/5 border-black/10 focus:border-emerald-600/30 text-slate-900 placeholder:text-slate-500'}`} /></div><button onClick={() => setShowFilters(!showFilters)} className={`p-3 rounded-xl border transition-all ${btnHover} ${showFilters ? (isDarkMode ? 'bg-vet-mint border-vet-mint text-charcoal' : 'bg-emerald-600 border-emerald-600 text-white') : (isDarkMode ? 'bg-white/5 border-white/10 text-white/40' : 'bg-black/5 border-black/10 text-black/40')}`}><SlidersHorizontal size={18} /></button></div>
                ...
                {/* Scroll container with extra bottom padding - UPDATED PADDING */}
            ...
            <div className={`absolute inset-0 flex flex-col bg-inherit transition-all duration-500 ease-in-out ${selectedClinic && !isAddingClinic ? 'opacity-100 scale-100 z-10 pointer-events-auto' : 'opacity-0 pointer-events-none scale-95'}`} style={{touchAction: 'pan-y'}}>
                {selectedClinic && (
                    <>
                        ...
                        {/* UPDATED: Details view scroll container */}
