...
export default function MapContainer() {
  ...
  // --- CRITICAL MOBILE FIX: Use explicit bottom anchors instead of height ---
  const getSidebarClass = () => {
      const base = `absolute z-20 flex flex-col transition-all duration-700 ease-[cubic-bezier(0.32,0.72,0,1)]`;
      const desktop = "md:inset-auto md:right-6 md:top-6 md:bottom-6 md:w-125 md:h-auto md:rounded-4xl md:translate-y-0";
      
      let mobile = "";
      // The secret sauce: top-45dvh AND bottom-0 forces the browser to respect the viewport boundary
      if (selectedClinic) { mobile = "inset-x-0 bottom-0 top-[45dvh] rounded-t-3xl pointer-events-auto"; }
      else if (showMobileMap) { mobile = "inset-0 rounded-none pointer-events-auto"; }
